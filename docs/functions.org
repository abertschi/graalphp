...
There can only be one function definition independent on the scope. One function can only exist with the same name
(in the same namespace, but we ignore name spaces for now).
