...
- We run this benchmark twice. Once with explicit copy-by-ref semantics for
  arrays, and one version which embodies copy-by-value which is the default for
  PHP. By-reference results show what a more sophisticated array implementation
  which may implement copy-on-write or similar techniques can achieve.
